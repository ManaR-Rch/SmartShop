...
/**
 * Tests pour valider les scénarios d'acceptation du système de fidélité (tier
 * promotion).
 * 
 * Les statistiques (totalOrders, totalSpent) sont maintenant persistées
 * directement
 * dans l'entité Client lors de chaque confirmation de commande via
...
class TierPromotionTest {
   ...
   * 
   * Les stats sont déjà persistées en base (mises à jour par
   * OrderService.confirmOrder)
   * Vérifier que calculateAndUpdateTier() utilise les stats persistées et les
   * convertit en tier.
   */
  @Test
