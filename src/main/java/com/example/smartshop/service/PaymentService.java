...
public class PaymentService {
    ...
    public PaymentResponseDTO addPayment(PaymentRequestDTO dto) {
        ...
        if (dto.getMethod() == PaymentMethod.CASH) {
            if (dto.getAmount() > CASH_LEGAL_LIMIT) {
                throw new BusinessRuleViolationException(
                    "Cash payment (" + dto.getAmount() + ") exceeds legal limit of " + CASH_LEGAL_LIMIT + " DH (Article 193 CGI)"
                );
            }
        }
     ...
     * 
     * When a payment is marked as ENCAISSE, the order's remainingAmount is recalculated
